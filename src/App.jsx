...
Génère le code complet pour répondre à cette description.`;
  ...
  }, [apiKey, description, getSystemPrompt]);
  ...
  const artifactToXml = useCallback((artifact) => {
    let xml = '<div class="__boltArtifact__" data-message-id="u3SxlJm0Xh4SwxHN"></div>`;
    }
      ...
      {artifact && artifact !== 'Error' && (
        <div className="mt-4">
          ...
          {!isWebContainerBooted ? (
            ...
            <button
              className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
              onClick={() => startWebContainer(JSON.parse(artifact))}
              disabled={isGenerating}
            >
